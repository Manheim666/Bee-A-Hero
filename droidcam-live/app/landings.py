...
class FlowerRegistry:
    """Assign sticky ids to per-frame flower boxes by IoU match to the previous frame."""

    def __init__(self, iou_thresh: float = 0.3, forget_s: float = 3.0) -> None:
    ...
    def update(self, boxes: list, t_s: float) -> list:
        # forget flowers unseen for a while so ids do not drift forever
        for fid in [f for f, v in self._flowers.items() if t_s - v["last_t"] > self._forget]:
        ...
        for box in boxes:
            ...
            if best is None:
                ...
                self._next += 1
            self._flowers[best] = {"box": box, "last_t": t_s}
            used.add(best)
...
class LandingLogger:
    ...
    def observe(self, insect_tracks: list, flower_boxes: list) -> list:
        ...
        for tid, box, typ, conf in insect_tracks:
            ...
            cen = _center(box)
            area = _area(box)
            sub = self._state.setdefault(tid, {"prev_c": cen, "prev_t": t_s, "ep": None,
                                               "votes": Counter(), "bee": Counter()})
            ...
            if settled:
                ...
                if ep is not None and t_s - ep["last_t"] > self._grace_s:
                    self._finalize(tid)
        # tracks that vanished this frame -> close once past the grace window
        for tid, sub in list(self._state.items()):
            ep = sub.get("ep")
            if tid not in present and ep is not None and t_s - ep["last_t"] > self._grace_s:
                self._finalize(tid)
            elif tid not in present and ep is None and t_s - sub["prev_t"] > 5.0:
                del self._state[tid]           # drop stale, landing-less tracks
        return flowers

    def _finalize(self, tid: int) -> None:
        sub = self._state.get(tid)
        if not sub or not sub.get("ep"):
            return
        ep = sub["ep"]
        landing_s = round(ep["last_t"] - ep["enter_t"], 2)
        typ = sub["votes"].most_common(1)[0][0] if sub["votes"] else "insect"
        row = {
            ...
            "flower_id": ep["flower"], "track_id": tid, "insect_type": typ,
